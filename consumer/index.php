								...
								<div class="m-sm-4">
									...
									<?php
										if(isset($_POST['submit']))
										{
											...
											$sql="SELECT * FROM consumer WHERE consumer_username='$username' AND consumer_password='$password'";
											$query=mysqli_query($con,$sql) or die(mysqli_error($con));
											if(mysqli_num_rows($query))
											{
												$fetch=mysqli_fetch_array($query);
												$_SESSION['consumer_id']=$fetch['consumer_id'];
												$_SESSION['consumer_name']=$fetch['consumer_name'];
												$_SESSION['consumer_email']=$fetch['consumer_email'];
												?>
													<script>
														Swal.fire(
														{
